...
class Post {
  ...
  /**
   * Renderiza una plantilla (scaffold) para un nuevo post utilizando front-matter.
   *
   * Este método obtiene una plantilla base (scaffold) para un tipo específico de contenido
   * (layout) y procesa los datos del post o página para generar un contenido completo.
   * Combina el front-matter y el contenido del post, y ajusta según el formato (JSON o YAML).
   * Devuelve una promesa con el contenido generado, que incluye tanto los datos
   * del front-matter como el cuerpo del post.
   *
   * @param {PostData} data - Un objeto que contiene los datos del post o página,
   * incluyendo layout, título, fecha, slug, entre otros metadatos.
   *
   * @returns {Promise<string>} - Una promesa que se resuelve con el contenido renderizado,
   * incluyendo tanto el front-matter como el cuerpo del post.
   *
   * @throws {Error} - Si ocurre un error al obtener la plantilla o al procesar los datos del front-matter.
   */
